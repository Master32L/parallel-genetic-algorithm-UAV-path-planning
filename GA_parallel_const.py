...
def parallel_GA_const(cluster, pop_size=1024, tpb=32, seed=1, num_iter=100,
    ...
    @cuda.jit
    def fitness_kernel(d_pop, d_fitness_all):
        ...
        if i < pop_size:
            fitness = 0
            for j in range(m-1):
                fitness += distance(d_pop[i,j], d_pop[i,j+1])
    ...
    def distance(cp1, cp2):
        ...
        """
        d_coordinates = cuda.const.array_like(coordinates)
    ...
    @cuda.jit
    def local_opt_kernel(d_next_gen, d_is_elite):
    ...
    @cuda.jit(device=True)
    def local_opt_one_trial(d_next_gen, i):
    ...
    for i in range(num_iter):
        # evaluate fitness
        fitness_kernel[tpb, bpg](d_pop, d_fitness_all)
        # sort
        ...
        if i < 32:
            local_opt_kernel[tpb,bpg](d_next_gen, d_is_elite)
        d_pop = d_next_gen
